...
const getQuestions = expressAsyncHandler(async (req, res) => {
  const { topic_id, category } = req.body;
...
const gettopics = expressAsyncHandler(async (req, res) => {
  const topics = await Question.distinct("topic_id");
  if (topics) {
    res.status(200).send(topics);
  } else {
    res.status(500);
    throw new Error("Failed to find all the topics in the database");
  }
});

module.exports = { getQuestions, getAnswers, gettopics };
